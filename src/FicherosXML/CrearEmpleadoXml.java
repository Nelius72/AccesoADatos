...
public class CrearEmpleadoXml {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            DocumentBuilder builder = factory.newDocumentBuilder(); //Creamos un xml
            DOMImplementation implementation = builder.getDOMImplementation(); //Creamos el Document con nodo raiz
            Document document = implementation.createDocument(null, "Empleados", null);//Le damos nombre
            document.setXmlVersion("1.0");

            for (; ; ) { //Bucle infinito hasta que le indicamos el break? Linea55
                file.seek(posicion); //nos posicionamos
                id = file.readInt();   // obtengo id de empleado
                for (int i = 0; i < apellido.length; i++) { //Recorro el espacio de apellido y voy almacenando cada caracter en un espacio
                    aux = file.readChar();
                    apellido[i] = aux;
                }
